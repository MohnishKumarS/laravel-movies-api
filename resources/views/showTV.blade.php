    ...
    <section class="show-moviebg">
        <div class="shows container">
            <div class="row mb-5 pt-5">
                <div class="col-lg-5">
                    <div>
                        <img src="https://image.tmdb.org/t/p/w500/{{ $viewShow['poster_path'] }}" class="img-fluid shadow"
                            alt="">
                    </div>
                </div>
                <div class="col-lg-7 mt-4 mt-lg-0">
                    <div>
                        <h2>{{ $viewShow['name'] }}
                            ({{ Carbon\Carbon::parse($viewShow['first_air_date'])->format('Y') }})</h2>
                        @php
                            echo $viewShow['tagline'] ? "<p class=''>{$viewShow['tagline']} </p>" : '';
                        @endphp
                        <p class="small"><i class="fa-solid fa-star text-warning"></i>
                            {{ floor($viewShow['vote_average'] * 10) }}% |
                            {{ Carbon\Carbon::parse($viewShow['last_air_date'])->format('M d, Y') }} |
                            @foreach ($viewShow['genres'] as $item)
                                {{ $item['name'] . ',' }}
                            @endforeach
                        </p>
                        <h5 class="text-warning">Overview</h5>
                        <p class="text-secondary">{{ $viewShow['overview'] }}</p>

                        <h5 class="text-warning">Features cast</h5>

                        <div class="row mt-3">
                            @foreach ($viewShow['credits']['crew'] as $crew)
                                @if ($loop->index < 4)
                                    <div class="col-3">
                        ...
                        <div x-data="{ isOpen = false }">
                            @if (count($viewShow['videos']['results']) > 0)
                                <div class="mt-4 ">
                                    ...
                                    {{-- <a href="https://youtube.com/watch?v={{ $viewShow['videos']['results'][0]['key'] }}"
        ...
        <div class="cast container my-5">
            ...
            <div class="row">
                @foreach ($viewShow['credits']['cast'] as $cast)
                    @if ($loop->index < 8)
        ...
        <div class="movie-img container my-5" x-data="{ isOpen: false }">
            ...
            <div class="row row-cols-2 row-cols-lg-4 mb-5">
                @foreach ($viewShow['images']['posters'] as $imgs)
                    @if ($loop->index < 4)
                        <div class="col ">
                            <div>
                                <a @click.prevent = "
            ...
            <div class="row row-cols-2 row-cols-lg-4 mb-4">
                @foreach ($viewShow['images']['backdrops'] as $imgs)
                    @if ($loop->index < 4)
                        <div class="col ">
                            <div>
                                <a @click.prevent = "
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="videoWrapper">
                        ...
                        src="https://www.youtube.com/embed/{{ $viewShow['videos']['results'][0]['key'] }}"
...
@push('styles')
    <style>
        ...
        .show-moviebg {
            position: relative;
            background: url("https://image.tmdb.org/t/p/original/{{ $viewShow['images']['backdrops'][0]['file_path'] }}");
